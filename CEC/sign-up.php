...
<head>
    ...
    margin-top: 20px; /* Adjust this value to move it down slightly */
}
    ...
    <section>
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (empty($enrollErr) && empty($nameErr) && empty($dobErr) && empty($timeErr) && empty($genderErr)) {
                ...
                header("Location: details.php?enroll=" . urlencode($enroll) . "&name=" . urlencode($name) . "&dob=" . urlencode($dob) . "&degree=" . urlencode($_POST['Degree']) . "&gender=" . urlencode($gender) . "&hobbies=" . urlencode(implode(',', $_POST['hobbies'])) . "&time=" . urlencode($time));
                exit();
            }
        }
        ?>
        ...
        <div class="container">
            ...
            <div class="login">
                Already have an account? <a href="3_log-in.php">Login</a>
            </div>
        </div>
